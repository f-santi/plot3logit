...
author:
  - name: Flavio Santi
    orcid: 0000-0002-2014-1981
    affiliation: 'University of Trento'
    address: |
      | Department of Economics and Management
      | Via Inama 5
      | 38122 Trento (TN), Italy
    email: \email{flavio.santi@unitn.it}
    url: www.flaviosanti.it
  - name: Maria Michela Dickson
    orcid: 0000-0002-4307-0469
    affiliation: 'University of Trento \AND'
  - name: Giuseppe Espa
    orcid: 0000-0002-0331-3630
    affiliation: 'University of Trento'
  - name: Diego Giuliani
    orcid: 0000-0002-7198-6714
    affiliation: 'University of Trento'
title:
  formatted: "\\pkg{plot3logit}: Ternary Plots for Interpreting Trinomial Regression
Models"
  # If you use tex in the formatted title, also supply version without
  plain:     "plot3logit: Ternary Plots for Interpreting Trinomial Regression
Models"
  # For running headers, if needed
  short:     "\\pkg{plot3logit}: Ternary Plots for Interpreting Trinomial Regression
Models"
abstract: >
...
keywords:
  ...
  formatted: [plotting software, ternary diagrams, "\\proglang{R}", "\\pkg{plot3logit}"]
  plain:     [plotting software, ternary diagrams, R, plot3logit]
header-includes:
  - \usepackage{amsmath}
  - \usepackage{amsfonts}
  - \usepackage{tikz}
...
vignette: >
  %\VignetteIndexEntry{Ternary Plots for Interpreting Trinomial Regression Models}
...
As~\eqref{eq:Delta} shows, the probability distribution after the covariate
change \(\Delta\) only depends on the probability distribution before change
\(\pi^{(m)}_{(x_0)}\) (\(m=1,2,3\)), and on the coefficients of the trinomial
regression, whereas there is not dependence on \(x_0\) other than through
\(\pi^{(m)}_{(x_0)}\). Relation~\eqref{eq:Delta} is thus the theoretical basis
which justifies the graphical method proposed in \citet{santi2019}, as it allows
one to represent and analyse the regression coefficients
...
those functions.\footnote{The vignette of the package illustrates some examples
...
\begin{figure}[t]
\centering
```{r, echo=FALSE, fig=TRUE, height = 4, width=4}
ptsAB <- list(A = c(0.3, 0.4, 0.3), B = c(0.5, 0.1, 0.4))
par(mfrow = c(2, 2), cex = 0.5, mar = rep(0, 4))
# Top-left
plot(field3logit(modVote, "genderFemale", edge = 0.1))
...
fortified object\break \code{fieldFemale}:\footnote{The seed of random number
...
are actually needed, the\break "\code{multifield3logit}" object can be subsetted
through the S3 method \code{"["}.

Finally, a peculiar behaviour of argument \code{label} is worth of being
reported. When a\break "\code{multifield3logit}" object is generated by
...
\pkg{plot3logit} and this article.
